...
def evaluate_model(train_df, test_df, df_recs, top_n: int = 5) -> dict:
    ...
    customer_overlap_ratio = (
        len(overlap_customers) / len(test_customers) * 100 if test_customers else 0
    )
    customer_cold_start_ratio = (
        len(only_test_customers) / len(test_customers) * 100 if test_customers else 0
    )
    ...
    product_cold_start_ratio = (
        len(only_test_products) / len(test_products) * 100 if test_products else 0
    )
    ...
    logger.info(
        f"   - New customer (Cold Start) ratio: {customer_cold_start_ratio:.2f}% (Data Mismatch Detected)"
    )
